...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x):
        ...
        # assert x.size(0) == 50
        # assert len(self.positional_embedding) == 50
...
class ModifiedResNet(nn.Module):
    ...
    def __init__(self, layers, output_dim, heads, input_resolution=224, width=64, with_attn=True, out_indices=[]):
        ...
        self.conv3 = nn.Conv2d(width // 2, width, kernel_size=3, padding=1, bias=False)  # Bx64x112x112
        ...
        self.layer1 = self._make_layer(width, layers[0])  # Bx64x64x64 -> Bx256x64x64
        self.layer2 = self._make_layer(width * 2, layers[1], stride=2)  # Bx256x64x64 -> Bx512x32x32
        self.layer3 = self._make_layer(width * 4, layers[2], stride=2)  # Bx512x32x32 -> Bx1024x16x16
