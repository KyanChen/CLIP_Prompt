...
att2types = {'id2type': {}, 'category2typeid': {}}
types = list(set([x["supercategory"] for x in type_data]))
for idx in range(len(types)):
    att2types['id2type'][idx] = types[idx]
for item in type_data:
    att2types['category2typeid'][item['name']] = types.index(item['supercategory'])
att2types['num_category'] = len(att2types['category2typeid'])
