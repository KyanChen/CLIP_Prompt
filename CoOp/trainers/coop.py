...
class DatasetWrapper(TorchDataset):
    ...
    def __init__(self, cfg, data_source, transform=None, is_train=False):
        ...
        trans = []
        trans += [T.ToTensor()]
        trans += [T.RandomResizedCrop(size=cfg.INPUT.SIZE, scale=(0.6, 1))]
        trans += [T.RandomHorizontalFlip()]
        if "normalize" in cfg.INPUT.TRANSFORMS:
            normalize = T.Normalize(
                mean=cfg.INPUT.PIXEL_MEAN, std=cfg.INPUT.PIXEL_STD
            )
    ...
    def __getitem__(self, idx):
        ...
        img0 = np.array(read_image(impath))
        ...
        x1 = x + (2+ratio)*w / 2 
        y1 = y + (2+ratio)*h / 2 
        x0, x1 = np.clip([x0, x1], 0, max_w)
        y0, y1 = np.clip([y0, y1], 0, max_h)
        # cv2.imshow('1', img0)
        # cv2.waitKey(0)
        img0 = img0[int(y0):int(y1), int(x0):int(x1), :]
        img = self._transform_image(self.trans, img0)
        output["img"] = img
...
class CoOp(TrainerX):
    ...
    def build_data_loader(self):
        ...
        dm = DataManager(self.cfg, dataset_wrapper=DatasetWrapper)
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            ...
            loss = F.binary_cross_entropy_with_logits(tmp_output, tmp_label, reduction='none')
